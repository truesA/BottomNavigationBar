...
import com.lhm.view.bottomnavigation.behaviour.BottomNavBarFabBehaviour;
import com.lhm.view.bottomnavigation.behaviour.BottomVerticalScrollBehavior;
import com.lhm.view.bottomnavigation.utils.Utils;
...
public class BottomNavigationBar extends FrameLayout {
    ...
    public static final int MODE_FIXED_EX = 5;

    @IntDef({MODE_DEFAULT, MODE_FIXED, MODE_SHIFTING, MODE_FIXED_NO_TITLE, MODE_SHIFTING_NO_TITLE, MODE_FIXED_EX})
...
//    }
    ...
    public void initialise() {
        ...
        if (!mBottomNavigationItems.isEmpty()) {
            ...
            }else if (mMode==MODE_FIXED_EX){//加强
                int[] widths = BottomNavigationHelper.getMeasurementsForFixedMode(getContext(), screenWidth, mBottomNavigationItems.size(), mScrollable);
                int itemWidth = widths[0];

                for (BottomNavigationItem currentItem : mBottomNavigationItems) {
                    FixedExBottomNavigationTab bottomNavigationTab = new FixedExBottomNavigationTab(getContext());
                    //mMode == MODE_FIXED_NO_TITLE 为false 表示不隐藏文字
                    setUpTab(mMode == MODE_FIXED_NO_TITLE, bottomNavigationTab, currentItem, itemWidth, itemWidth);
                }
            }
